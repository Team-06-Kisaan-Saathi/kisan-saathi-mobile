  ...
  return (
    <View style={[styles.header, highContrast && { backgroundColor: "#000", borderBottomColor: "#333" }]}>
      ...
      <Text style={styles.headerSub}>
        {t("market.sub") || t("market.subtitle") || "Today’s mandi prices, nearby markets & comparisons"}
      </Text>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
        <Text style={styles.headerHint}>
          {t("market.loc_src") || t("market.location_source") || "Location source:"} {coordsSource.toUpperCase()}
        </Text>
        <Pressable onPress={onUpdateLocation}>
          <Text style={{ color: '#2d6a4f', fontSize: 12, fontWeight: '700' }}>{t("market.update_location") || "Update Location"}</Text>
        </Pressable>
      </View>
    </View>
  );
